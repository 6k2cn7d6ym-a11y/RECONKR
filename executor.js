...
async function main(){
  ...
  // 3. orders 파일 (없으면 휴장 또는 bot 미실행)
  var ordersFile = path.join(CFG.ordersDir, date + '.json');
  if(!fs.existsSync(ordersFile)){
    console.log('[SKIP] orders/' + date + '.json 없음 — 휴장 또는 bot 미실행'); return;
  }
  ...
  for(var ei = 0; ei < exits.length; ei++){
    ...
    var entryNotional = exPos ? exPos.entry * exOrder.qty : 0;
